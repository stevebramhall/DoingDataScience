...
####For every 1 IBU increase, the estimated ABV increases 3.676e-04.
...
####For every 1 IBU increase, the estimated ABV increases 4.172e-04.
...
```
####We are 95% confident for every 1 IBU increase, the ABV increase has a plausible range of 2.997e-04 to 4.354e-04. 
#### TEXAS
```{r echo = TRUE}
confint(fitTX,level=.95)
```

####We are 95% confident for every 1 IBU increase, the ABV increase has a plausible range of 3.44e-04 to 4.904e-04. 
...
fitTrainCO1 = lm(AlcoholByVolume~BitternessUnits, data = trainCO)
...
fitTrainCO2 = lm(AlcoholByVolume~BitternessUnits + IBU2, data = trainCO)
...
if (MSE_CO1 < MSE_CO2) { 
  print("The linear model is better than the quadratic model for Colorado AVB vs IBU")
}else { print("The quadratic model is better than the linear model for Colorado AVB vs IBU") }

```
####Models fit for TX
```{r}
#Fit the linear model for TX
fitTrainTX1 = lm(AlcoholByVolume~BitternessUnits, data = trainTX)
...
#Fit the quadratic model for cO
fitTrainTX2 = lm(AlcoholByVolume~BitternessUnits + IBU2, data = trainTX)
...
if (MSE_TX1 < MSE_TX2) { 
  print("The linear model is better than the quadratic model for Texas AVB vs IBU")
}else { print("The quadratic model is better than the linear model for Texas AVB vs IBU") }
```
