...
#### Set Search Parmeters. "Central Park Jogger", DATES = 1989-1999
```{r}
...
# For knit timing delay
#for(i in 1:100000000) { j = (i + 1 -1 )/i }

pages <- list()       # create place holder
for(i in 0:maxPages){
...
#Split data into News and Other
...
#  print(key_word)
  ...
  # search in snippets
  pKWGivenNews = length(grep(key_word,NewsGroup$response.docs.snippet,ignore.case = TRUE))/dim(NewsGroup)[1]    #P(keyword | News)
  pKWGivenOther = length(grep(key_word,OtherGroup$response.docs.snippet,ignore.case = TRUE))/dim(OtherGroup)[1] #P(keyword | Other)

  pKW = length(grep(key_word,trainingSet$response.docs.snippet,ignore.case = TRUE))/dim(trainingSet)[1]         #P(keyword)

  pNewsGivenKW = pKWGivenNews*pNews/pKW                                                                         #P(News | keyword)
  pOtherGivenKW = pKWGivenOther*pOther/pKW                                                                      #P(Other | keyword)
...
#strsplit(allNYTSearch$response.docs.headline.main[i],split = " ")
  ...
  theText = unlist(strsplit(gsub("[^[:alnum:] ]", "", allNYTSearch$response.docs.snippet[i]), " +")) # get snippet text
  for(j in 1 : length(theText))  #This loop iterates over each word in the snipet 
  {
#    articleScoreNews = articleScoreNews + Pnews_word(theText[j],allNYTSearch) # send to training set, adds P(News|keyword)
#    articleScoreOther = articleScoreOther + (1 - Pnews_word(theText[j],allNYTSearch))
    articleScoreNews = articleScoreNews + Pnews_word(key_word,allNYTSearch) # send to training set, adds P(News|keyword)
    articleScoreOther = articleScoreOther + (1 - Pnews_word(key_word,allNYTSearch))
...
##### (3) Provide external cross validation (50%/50%). Create a training & test set from the total number of articles-randomly. Train the classifier on the training set and create the confusion matrix from the test set.
